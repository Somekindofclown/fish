...
iptables -A INPUT -p tcp --dport 22 -d 172.20.242.150 -m state --state new -j ACCEPT
...
iptables -A OUTPUT -p udp --dport 53 -d 172.20.240.20 -m state --state new -j ACCEPT

iptables -A OUTPUT -p tcp --dport 22 -d 172.20.242.150 -m state --state new -j ACCEPT

iptables -A OUTPUT -p udp --dport 123 -d 172.20.240.20 -m state --state new -j ACCEPT
